...
msg['From'] = from_addr
msg['To'] = to_addr
msg['Subject'] = Header('Toronto Covid-19 report: '+myDateString, 'utf-8').encode()


reportData = open('GTADailyData.txt').read()


# attache a MIMEText object to save email content
msg.attach(MIMEText('<html><body><h1>Greater Toronto Covid19 Report for ' +myDateString+'</h1>' +
"<h2>"+
reportData +
"</h2>"+
'<p><img src="cid:0"></p>' +
...
with open('GTACovid2weekline.png', 'rb') as f:
    # set attachment mime and file name, the image type is png
    mime = MIMEBase('image', 'png', filename='2week.png')
    # add required header data:
    mime.add_header('Content-Disposition', 'attachment', filename='GTACovid2weekline.png')
...
# to add an attachment is just add a MIMEBase object to read a picture locally.
with open('GTACovid2weekAgeHist.png', 'rb') as f:
    # set attachment mime and file name, the image type is png
    mime = MIMEBase('image', 'png', filename='2week.png')
    # add required header data:
    mime.add_header('Content-Disposition', 'attachment', filename='GTACovid2weekAgeHist.png')
